...
│   ├── public/          # Built frontend (generated by `npm run build`, gitignored)
...
│   │   ├── styles/      # CSS per page/component (colour tokens in styles/index.css)
...
```bash
git clone https://github.com/MRMUHAMMADHAMZA/kainos-portal.git
cd kainos-portal
```
...
For deployment, the backend can build and serve the frontend itself, so the whole app runs from one origin/port (this is how it's deployed):

```bash
cd backend
npm install
npm run build   # builds ../frontend and copies the output into backend/public
npm start       # serves the API and the built frontend from http://localhost:5000
```

The frontend's API client calls a relative `/api` path, so frontend and backend must be served from the same origin in production — either via this single-server setup, or a reverse proxy that routes `/api/*` to the backend if you host them separately.
